...
namespace MBS.Web
{
	public static class ExtensionMethods
	{
		...
		public static void RegisterScript(this System.Web.UI.Page page, string fileName)
		{
			...
			{
				if (((control as System.Web.UI.HtmlControls.HtmlGenericControl)?.TagName) == "script")
				{
					if (((System.Web.UI.HtmlControls.HtmlGenericControl)control).Attributes["src"] == page.ExpandRelativePath(fileName))
					{
						return;
					}
				}
		...
		public static bool ContainsCssClass(this System.Web.UI.HtmlControls.HtmlControl ctl, string className)
		{
			string attClass = ctl.Attributes["class"];
			if (attClass != null)
			{
				string[] cssClasses = attClass.Split(new char[] { ' ' });
				return Array.Exists<string>(cssClasses, new Predicate<string>((string obj) => className.Equals(obj)));
			}
			return false;
		}
		public static void AddCssClass(this System.Web.UI.HtmlControls.HtmlControl ctl, string className)
		{
			if (ctl.ContainsCssClass(className)) return;
			ctl.Attributes["class"] = ApplyCssClass(ctl.Attributes["class"], className);
		}
		...
		private static string ApplyCssClass(string original, string newclass)
		{
			...
			string[] newCssClassesSpecified = newclass.Split(new char[] { ' ' });
			string[] newCssClasses = new string[cssClasses.Length + newCssClassesSpecified.Length];
			Array.Copy(cssClasses, 0, newCssClasses, 0, cssClasses.Length);
			Array.Copy(newCssClassesSpecified, 0, newCssClasses, cssClasses.Length, newCssClassesSpecified.Length);
